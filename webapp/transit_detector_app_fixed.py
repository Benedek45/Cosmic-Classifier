...
MODELS = {
    ...
    'attention': MODEL_DIR / 'attention_best.pt',
    'ensemble': [MODEL_DIR / 'cnn_best.pt', MODEL_DIR / 'attention_best.pt']
}
...
def predict_telescope():
    ...
    try:
        ...
        if is_ensemble:
            ...
            attention_results = individual_results[1]  # Second model is Attention

            cnn_detection_rate = cnn_results['detections'] / len(windows)
            attention_detection_rate = attention_results['detections'] / len(windows)

            # Simple threshold: >30% detection rate means model says "transit"
            cnn_says_transit = cnn_detection_rate > 0.3
            attention_says_transit = attention_detection_rate > 0.3

            # OR logic: Either model detecting → verdict is transit
            is_transit = cnn_says_transit or attention_says_transit
            ...
            if cnn_says_transit and attention_says_transit:
                agreement_indicator += ' (Both models detected)'
            elif cnn_says_transit:
                agreement_indicator += ' (CNN detected, Attention did not)'
            elif attention_says_transit:
                agreement_indicator += ' (Attention detected, CNN did not)'
        else:
